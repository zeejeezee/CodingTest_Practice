...
public class ex8 {
    ...
    //나머지 한 변이 될 수 있는 정수의 개수를 return하도록 solution 함수를 완성해 주세요.
    
    public int solution9(int[] sides) {
        int valueMax = Math.max(sides[0], sides[1]);
        int valueMin = Math.min(sides[0], sides[1]);
        int highValue = valueMax + valueMin;
        int lowValue = valueMax - valueMin;
    ...
    //존재하지 않는다면 2를 return하도록 solution 함수를 완성해 주세요.
    
    public int solution10(String[] spell, String[] dic) {
